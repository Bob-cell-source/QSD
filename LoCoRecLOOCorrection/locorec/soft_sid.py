...
class SoftSIDConfig:
    ...
    min_conditional_lift: float = 0.0
...
def build_soft_sid_table(
    ...
    correction_features = torch.zeros(
        num_items + 1, depth, 5, dtype=torch.float
    )
    slot_counts: List[Counter[int]] = [Counter() for _ in range(depth)]
    first_item = min(item_codes)
    slot_offsets = [
        int(semantic_table[first_item, slot]) - item_codes[first_item][slot]
    ...
    for item in range(1, num_items + 1):
        ...
        for slot in range(depth):
            ...
            for token, count in counts.items():
                if token <= 0:
                    continue
                local_probability = count / neighbor_count
                raw_token = int(token - slot_offsets[slot])
                global_probability = slot_counts[slot][raw_token] / num_items
                lift = math.log(
                    (local_probability + 1e-8)
                    / (global_probability + 1e-8)
                )
                support[token] = local_probability
                lifts[token] = lift
            non_hard = sorted(
                (
                    (token, value)
                    ...
                    if token != hard_token
                    and value >= config.min_support
                    and lifts[token] > config.min_conditional_lift
                ),
                key=lambda row: (row[1] * math.exp(lifts[row[0]]), row[1], row[0]),
                reverse=True,
            ...
            alternative_support = max(
                (value for _, value in non_hard), default=0.0
            )
            prior_entropy = float(
                -(weights * torch.log(weights.clamp_min(1e-8))).sum()
            )
            if len(ranked) > 1:
                prior_entropy /= math.log(len(ranked))
            else:
                prior_entropy = 0.0
            correction_features[item, slot] = torch.tensor(
                [
                    math.log1p(neighbor_count),
                    hard_support,
                    alternative_support,
                    alternative_support - hard_support,
                    prior_entropy,
                ],
                dtype=torch.float,
            )
