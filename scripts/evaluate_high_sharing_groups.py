...
def parse_bucket_spec(spec: str):
    ...
    if not buckets:
        raise ValueError("No valid group-size buckets.")
    return buckets


def bucket_name(group_size: int, buckets) -> str:
    for name, lower, upper in buckets:
        if group_size >= lower and (upper is None or group_size <= upper):
            return name
    return "other"


def build_prefix_group_sizes(
...
def evaluate_grouped(
    ...
    stats = {
        key: {"count": 0, **{f"HR@{k}": 0.0 for k in ks}, **{f"Recall@{k}": 0.0 for k in ks}, **{f"NDCG@{k}": 0.0 for k in ks}}
    ...
    for seq, targets in loader:
        ...
        for row_idx in range(batch_size):
            ...
            group_size = item_group_sizes.get(target, 1)
            bucket = bucket_name(group_size, buckets)
...
def main() -> None:
    ...
    parser.add_argument("--prefix-level", type=int, default=None)
    ...
    item_group_sizes = build_prefix_group_sizes(item_semantic_ids, prefix_level)
