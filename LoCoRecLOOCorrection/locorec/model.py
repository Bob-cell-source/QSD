...
class LoCoRecItemEncoder(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "correction_features", correction_features.float()
        )
        self.register_buffer(
        ...
        correction_hidden = max(8, dim // 8)
        self.soft_correction_gate = nn.Sequential(
            nn.Linear(5, correction_hidden),
            nn.GELU(),
            nn.Linear(correction_hidden, 1),
        )
        with torch.no_grad():
            self.soft_correction_gate[-1].weight.zero_()
            self.soft_correction_gate[-1].bias.fill_(-2.0)
    ...
    def hard_centered_semantic_pool(
        self,
        ...
        hard = vectors[..., 0, :]
        alternative_mask = tokens[..., 1:].ne(0)
        alternative_weights = weights[..., 1:] * alternative_mask
        alternative_weights = alternative_weights / alternative_weights.sum(
            dim=-1, keepdim=True
        ).clamp_min(1e-8)
        alternative = (
            vectors[..., 1:, :] * alternative_weights.unsqueeze(-1)
        ).sum(dim=-2)
        has_alternative = alternative_mask.any(dim=-1)
        correction_strength = torch.sigmoid(
            self.soft_correction_gate(self.correction_features[items])
        ).squeeze(-1)
        correction_strength = correction_strength * has_alternative
        corrected = hard + correction_strength.unsqueeze(-1) * (
            alternative - hard
        )
        slot_mask = tokens[..., 0].ne(0).unsqueeze(-1).float()
        pooled = (corrected * slot_mask).sum(dim=-2) / slot_mask.sum(
            dim=-2
        ).clamp_min(1.0)
        valid_slots = slot_mask.squeeze(-1).sum().clamp_min(1.0)
        correction_mean = (
            correction_strength * slot_mask.squeeze(-1)
        ).sum() / valid_slots
        return pooled, correction_mean
    ...
    def forward(self, items: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        basis_pool, basis_correction = self.hard_centered_semantic_pool(
            self.semantic_basis_embedding, items, candidate_weights
        )
        basis = self.basis_projection(
            basis_pool
        )
        shared, shared_correction = self.hard_centered_semantic_pool(
        ...
        return {
            ...
            "gate_mean": gate_mean,
            "soft_correction_mean": 0.5
            * (basis_correction + shared_correction),
        }
    ...
    def statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "soft_correction_mean": float(
                (
                    torch.sigmoid(
                        self.soft_correction_gate(
                            self.correction_features[items]
                        )
                    ).squeeze(-1)
                    * self.soft_sid_table[items, :, 1:].ne(0).any(dim=-1)
                ).mean().cpu()
            ),
        }
    ...
    def forward(self, sequence: torch.Tensor, candidates: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            * (sequence_output["gate_mean"] + candidate_output["gate_mean"]),
            "soft_correction_mean": 0.5
            * (
                sequence_output["soft_correction_mean"]
                + candidate_output["soft_correction_mean"]
            ),
        }
