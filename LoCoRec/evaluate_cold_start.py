...
def build_model(checkpoint, device):
    ...
    soft_ids, candidate_prior, local_consistency = build_soft_sid_table(
        hard_table,
        item_codes,
        SoftSIDConfig(
    ...
    return model.to(device).eval(), sequences, frequency, num_items, cfg


@torch.no_grad()
def evaluate(model, sequences, frequency, num_items, cfg, args):
...
def main():
    ...
    groups = evaluate(model, sequences, frequency, num_items, cfg, args)
